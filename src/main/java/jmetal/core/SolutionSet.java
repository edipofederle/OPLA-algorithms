...
public class SolutionSet implements Serializable {
	...
	public void printObjectivesToFile(String path) {
		...
		try {
			...
			for (int i = 0; i < solutionsList_.size(); i++) {
				...
				bw.write(solutionsList_.get(i).toString().trim()
	...
	public void saveVariablesToFile(String path, List<FunResults> funResults,
			LogLog logger, boolean generate) {
		int numberOfVariables = solutionsList_.get(0).getDecisionVariables().length;

		if (logger != null)
			logger.putLog("Number of solutions: " + solutionsList_.size(),
					Level.INFO);
		for (int i = 0; i < solutionsList_.size(); i++) {
			for (int j = 0; j < numberOfVariables; j++) {
				...
				funResults.get(i).setSolution_name(
						pathToSave + arch.getName() + "-"
								+ funResults.get(i).getId());
				if (generate)
