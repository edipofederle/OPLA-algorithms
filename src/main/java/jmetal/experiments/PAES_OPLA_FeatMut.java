...
public class PAES_OPLA_FeatMut {
	...
	public String dirToSaveOutput; // Diretório que sera criado dentro do
									// diretorio configurado no arquivo de
									// configuracao
	...
	public void execute() throws FileNotFoundException, IOException,
			JMException, ClassNotFoundException {
		...
		this.numberObjectives = this.configs.getOplaConfigs()
		...
		for (String pla : plas) {
			...
			try {
				...
				this.configs.getLogger().putLog(
						String.format(
								"Error when try read architecture %s. %s",
								xmiFilePath, e.getMessage()));
			}

			Experiment experiement = mp.createExperimentOnDb(plaName, "PAES",
					configs.getDescription());
			ExperimentConfs conf = new ExperimentConfs(experiement.getId(),
					"PAES", configs);
			conf.save();
			...
			mutation = MutationFactory.getMutationOperator(
					"PLAFeatureMutation", parameters, this.configs);
			...
			List<String> selectedObjectiveFunctions = this.configs
					.getOplaConfigs().getSelectedObjectiveFunctions();
			mp.saveObjectivesNames(this.configs.getOplaConfigs()
					.getSelectedObjectiveFunctions(), experiement.getId());
			...
			for (int runs = 0; runs < runsNumber; runs++) {
				...
				List<FunResults> funResults = result.getObjectives(
						resultFront.getSolutionSet(), execution, experiement);
				List<InfoResult> infoResults = result.getInformations(
						resultFront.getSolutionSet(), execution, experiement);
				AllMetrics allMetrics = result.getMetrics(funResults,
						resultFront.getSolutionSet(), execution, experiement,
				...
				ExecutionPersistence persistence = new ExecutionPersistence(
						allMetricsPersistenceDependencies);
				try {
			...
			configs.getLogger().putLog(
					"------All Runs - Non-dominated solutions --------");
			List<FunResults> funResults = result.getObjectives(
			...
			AllMetrics allMetrics = result.getMetrics(funResults,
					todasRuns.getSolutionSet(), null, experiement,
					selectedObjectiveFunctions);
			mp.persisteMetrics(allMetrics, this.configs.getOplaConfigs()
			...
			DistanceEuclideanPersistence.save(
					c.calcula(this.experiementId, this.numberObjectives),
					this.experiementId);
			infoResults = null;
			funResults = null;
	...
	private void intializeDependencies() {
		...
		} catch (ClassNotFoundException | MissingConfigurationException
				| SQLException e) {
			e.printStackTrace();
		}

		allMetricsPersistenceDependencies = new AllMetricsPersistenceDependency(
				connection);
	...
	private void logInforamtions(String context, String pla) {
		...
		configs.getLogger().putLog("\tMuta -> " + mutationProbability,
				Level.INFO);
	...
	private void setDirToSaveOutput(String experimentID, String executionID) {
		...
		String dir;
		if (executionID != null) {
			dir = ReaderConfig.getDirExportTarget() + experimentID
					+ System.getProperty("file.separator") + executionID
	...
	private void saveHypervolume(String experimentID, String executionID,
			SolutionSet allSolutions, String plaName) {
		String dir;
		if (executionID != null)
			dir = ReaderConfig.getDirExportTarget() + experimentID + "/"
					+ executionID + "/Hypervolume/";
		else
			dir = ReaderConfig.getDirExportTarget() + experimentID
					+ "/Hypervolume/";
